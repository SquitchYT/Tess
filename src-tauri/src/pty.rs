...
impl Pty {
    pub fn new(app: Arc<AppHandle>, cmd: &str, cols: u16, rows: u16, id: String) -> Result<Self, PtyError> {
        ...
        if let Ok(pair) = pair {
            ...
            Err(PtyError::Create(String::from("An error occured when creating a new terminal.")))
        }
    }

    pub fn run(&mut self, cmd: &str, id: String, close_channel_receiver: Receiver<()>) -> Result<(), PtyError> {
        let cmd = CommandBuilder::from_argv(Vec::from_iter(cmd.split(' ').map(|s| std::ffi::OsString::from(s))));
                ...
                std::thread::spawn(move || {
                    loop {
                        if let Ok(Some(_)) = second_child_clone.as_ref().lock().unwrap().try_wait() {
                            cloned_app.emit_all("terminal_closed", id_cloned.clone()).ok();
                            break;
                        }
                    }
                });
    ...
    pub fn write(&mut self, content: String) -> Result<(), PtyError> {
        if let Ok(()) = write!(
            ...
            Err(PtyError::Write(String::from("pty doesn't accept the incoming data")))
        }
    }
    ...
    pub fn resize(&self, cols: u16, rows: u16) -> Result<(), PtyError> {
        if let Ok(()) = self.pair.master.resize(portable_pty::PtySize {
            ...
            Err(PtyError::Resize(String::from("pty doesn't accept the resize operation.")))
        }
    }
            ...
            if let Ok(mut child) = child.lock() {
                if let Ok(()) = child.kill() {
                    if let Ok(()) = self.close_channel_sender.send(()) {
                        ...
                        Err(PtyError::Kill(String::from("process didn't terminate correctly.")))
                    }
                ...
                Err(PtyError::Kill(String::from("doesn't have access to the pty.")))
            }
        } else {
            Err(PtyError::Kill(String::from("doesn't have access to the pty.")))
        }
