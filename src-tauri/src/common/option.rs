...
use crate::common::types::{
    BackgroundType, CursorType, PartialShortcutAction, RangedInt, ShortcutAction,
};

use super::types::{Macro, PartialMacro, PartialShortcut, Shortcut};
...
fn deserialize_theme<'de, D>(data: D) -> Result<String, D::Error>
where
    D: Deserializer<'de>,
{
    let theme_value: String = String::deserialize(data).unwrap();

    if std::path::Path::new(&format!(
        "{}/tess/themes/{}/style.css",
    ...
    } else if std::path::Path::new(&theme_value).exists() {
        Ok(theme_value)
    } else {
        Ok(String::new())
    }
    ...
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        ...
        for partial_profile in partial_option.profiles {
            let profile_option = TerminalOption {
                buffersize: partial_profile
                    .buffersize
                    .unwrap_or(partial_option.terminal.buffersize),
                cursor: partial_profile
                    .cursor
                    .unwrap_or(partial_option.terminal.cursor),
                fontsize: partial_profile
                    .fontsize
                    .unwrap_or(partial_option.terminal.fontsize),
            ...
            let mut profile_theme = partial_option.theme.clone();
            if let Some(partial_profile_theme) = partial_profile.theme {
                if std::path::Path::new(&format!(
                    "{}/tess/themes/{}/style.css",
                    dirs_next::config_dir().unwrap_or_default().display(),
                ...
                .exists()
                {
                    profile_theme = format!(
                        "{}/tess/themes/{}/style.css",
                        dirs_next::config_dir().unwrap_or_default().display(),
                        partial_profile_theme
                    )
                } else if std::path::Path::new(&partial_profile_theme).exists() {
                    profile_theme = partial_profile_theme
                }
            }
        ...
        if let Some(partial_option_shortcuts) = partial_option.shortcuts {
            for partial_shortcut in partial_option_shortcuts {
                match partial_shortcut.action {
                    ...
                    PartialShortcutAction::Copy => shortcuts.push(Shortcut {
                        shortcut: partial_shortcut.shortcut,
                        action: ShortcutAction::Copy,
                    }),
                    PartialShortcutAction::Paste => shortcuts.push(Shortcut {
                        shortcut: partial_shortcut.shortcut,
                        action: ShortcutAction::Paste,
                    }),
                    PartialShortcutAction::CloseAllTabs => shortcuts.push(Shortcut {
                        shortcut: partial_shortcut.shortcut,
                        action: ShortcutAction::CloseAllTabs,
                    }),
                    PartialShortcutAction::CloseFocusedTab => shortcuts.push(Shortcut {
                        shortcut: partial_shortcut.shortcut,
                        action: ShortcutAction::CloseFocusedTab,
                    }),
                    PartialShortcutAction::OpenDefaultProfile => shortcuts.push(Shortcut {
                        shortcut: partial_shortcut.shortcut,
                        action: ShortcutAction::OpenDefaultProfile,
                    }),
                    PartialShortcutAction::FocusFirstTab => shortcuts.push(Shortcut {
                        shortcut: partial_shortcut.shortcut,
                        action: ShortcutAction::FocusFirstTab,
                    }),
                    PartialShortcutAction::FocusLastTab => shortcuts.push(Shortcut {
                        shortcut: partial_shortcut.shortcut,
                        action: ShortcutAction::FocusLastTab,
                    }),
                    PartialShortcutAction::FocusNextTab => shortcuts.push(Shortcut {
                        shortcut: partial_shortcut.shortcut,
                        action: ShortcutAction::FocusNextTab,
                    }),
                    PartialShortcutAction::FocusPrevTab => shortcuts.push(Shortcut {
                        shortcut: partial_shortcut.shortcut,
                        action: ShortcutAction::FocusPrevTab,
                    }),
                    PartialShortcutAction::FocusTab(tab_index) => shortcuts.push(Shortcut {
                        shortcut: partial_shortcut.shortcut,
...
impl Default for PartialOption {
    fn default() -> Self {
        Self {
            theme: String::default(), // TODO: Set correct default
            background: BackgroundType::default(),
            custom_titlebar: true, // TODO: Set false on linux
            profiles: Vec::default(),
            terminal: TerminalOption::default(),
...
struct PartialProfile {
    ...
    theme: std::option::Option<String>, // TODO: Set correct them location here with serde deserialize with
