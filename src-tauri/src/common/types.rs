...
impl<'de, const MIN: u32, const MAX: u32, const DEF: u32> serde::Deserialize<'de> for RangedInt<MIN, MAX, DEF> {
...
impl<'de> serde::Deserialize<'de> for BackgroundType {
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        ...
        Ok(if let OptionRepresentation::Simple(option_value) = option_representation {
            match option_value.to_lowercase().as_str() {
                ...
                _ => {
                    if let Some(background_media) = BackgroundMedia::deserialize_from_string(option_value) {
